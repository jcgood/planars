...
#
# STEP 1 COPY (chart 6): everything below the header is copied verbatim from
# results/nyan1308_inton_laminar_forest.r, lines 1-37, at commit 43a308f --
# the R that scripts/analysis/laminar_analysis.py's generate_r_script()
# writes, for a forest of one tree. For N trees the generator repeats lines
# 10-25 once per tree (numbered variables), adds the visible label (lines
# 27-29) to the last tree only, and repeats the area() line N times. Wrapped
# in an uncalled function so loading the package doesn't run it. No other
# edits. Literals are replaced with bundle data in the next commit.

ghost_forest_step1_copy <- function() {
library(ape)
library(ggplot2)
library(ggtree)
library(patchwork)
