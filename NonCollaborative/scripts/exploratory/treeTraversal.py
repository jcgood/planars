...
# Module-level globals — reset at the start of each main() call so the
# function can be called more than once (e.g. for different domain files
# or domain-type subsets) without accumulating stale state.
domains = defaultdict(list)
trees = []
domainsCollapsed = []
treereductions = []
domainStrength = defaultdict(int)

def main(domain_file="domains_nyan1308.tsv",
         domains_dir=None,
         output_dir=None,
         subset=None,
         color="black",
         tpfx=""):
    """Run the full constituency-tree analysis for one domain file.

    Args:
        domain_file: TSV filename inside domains_dir.
        domains_dir: Path to the domains/ folder. Defaults to ../domains/
                     relative to this script's location.
        output_dir:  Where to write constituencyforest-all.r. Defaults to
                     the current working directory.
        subset:      List of Domain_Type strings to include (e.g.
                     ["phonological"]). None = all types.
        color:       Branch color for the ggtree output.
        tpfx:        Variable-name prefix for the R output (e.g. "phon").
    """
    # Reset global state so successive calls don't compound results.
    global domains, trees, domainsCollapsed, treereductions, domainStrength
    domains = defaultdict(list)
    trees = []
    domainsCollapsed = []
    treereductions = []
    ...
    domainRows = pandas.read_csv(os.path.join(domains_dir, domain_file), sep="\t")
    ...
    for index, row in domainRows.iterrows():
        ...
        if size != calculatedSize:
            raise ValueError(
                f"Size mismatch in {domain_file} row {index}: "
                f"stored Size={size} but Right_Edge - Left_Edge + 1 = {calculatedSize}. "
                f"Row: {row.to_dict()}"
            )
    ...
    rout = open(os.path.join(output_dir, tpfx + 'constituencyforest-all.r'), 'w')
...
if __name__ == "__main__":
    # To run a subset by domain type, pass subset= e.g.:
    #   main(subset=["phonological"], color="#4477AA", tpfx="phon")
    #   main(subset=["morphosyntactic"], color="#EE6677", tpfx="morsyn")
    #   main(subset=["tonosegmental", "intonational"], color="#228833", tpfx="tonoseg")
    main()