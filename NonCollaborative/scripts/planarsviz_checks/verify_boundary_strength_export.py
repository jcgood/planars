...
results/boundaries/nyan1308_boundary_strength.tsv, and
data/subsets/no_tono/boundary_strength.tsv against
results/boundaries/nyan1308_boundary_strength_no_tono.tsv, row for row.
...
for script_table, bundle_table in [
    ...
    a, b = read(NC / "results" / "boundaries" / script_table), read(bundle_table)
    same = a == b
    ok = ok and same
