...
The plain command reproduces the committed results/ files exactly (5000
draws, seed 0). Both summary TSVs record the draw count and seed that made
them, so a file can always say where its p-values came from.
"""
...
def main():
    ...
    # 5000 is what produced the committed results/ files, so the plain command
    # reproduces them.
