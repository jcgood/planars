...
## nyan1308_all_families_labeled.r → nyan1308_all_families_labeled.pdf / _legend.pdf
...
two-line ("position number" over "name") tip labels from `nyan1308_laminar_overlay.r`, using the
same invisible-per-tree-spacer + one-visible-label-on-top pattern. Generated by
`run_domain_overlay(overlay_groups=[(None, "black", "all")], alpha_divisor=1.0)` in
`laminar_analysis.py`, reusing `generate_r_overlay_script()` — the same function that generates
`nyan1308_laminar_overlay.r` below, just with one unfiltered group instead of five colored ones.
...
- **Darkness** = how many of the 69 families contain the span (structural robustness). This one
  ...
  mechanism. This is what makes the consensus backbone (see "laminar_conflict_groups.r" above)
...
`nyan1308_random_tree_overlay.r` further below — not the plain `offset=-1` still used (as of this
writing) in `nyan1308_laminar_overlay.r`, whose gap was measured too small (39px on a comparable
canvas) to read cleanly at normal viewing scale. Fixing `generate_r_overlay_script()` itself means
the fix now applies to any future regeneration of `nyan1308_laminar_overlay.r` too, though that
file's already-delivered `.pdf` hasn't been re-rendered as part of this change. The exact `vjust`
...
A domain-type color legend (the kind `nyan1308_laminar_overlay.r` gets, five colors, also inset)
would be meaningless on this all-black chart, so `generate_r_overlay_script()` branches: more than
one colored group in the call → color legend as before; exactly one → this darkness/thickness
legend instead.
...
## nyan1308_exemplary_trees.r → nyan1308_exemplary_trees_1.pdf .. _7.pdf
...
combines two selection ideas already named elsewhere in this project — `laminar_conflict_groups.r`'s
Panels B/C ("structurally diverse representatives... by greedy coverage") and
`laminar_four_trees.r`'s "most consensus-like family" — into one function, because neither exists
...
same selection. `generate_exemplary_trees(include_sparsest=True)` appends the family with the
...
`domain_charts-cgpt.r`'s own untouched `plot_height()` formula (`max(7, n_tests*0.7)` cm) — exactly
as legible as any of that script's standalone charts, not a scaled-down approximation. Placed side
...
**Reuses `df.plot()`/`constituency.plot()` from `domain_charts-cgpt.r` via `source()`** rather than
reimplementing the pooled-plot logic in Python-generated R — that function is the single source of
truth for what a pooled plot looks like, hand-maintained directly in `scripts/`. One consequence:
`source()`-ing that file re-runs it end to end, so generating this chart also re-saves all the
standard pooled-plot PDFs (`nyan1308_pooled_plot.pdf` and the rest) as a side effect — harmless
(same data in, same data out) but worth knowing about. Each family's own test set is read directly
off `Span.labels` (the original `Test_Labels` collected when spans are aggregated in `load_spans()`),
not re-derived, so the pooled plot beside each tree is guaranteed to show exactly the tests that
produced it.

**Must be run with `planars/` as the working directory** — `here::here()` (used both by the
`source()` call and inside `domain_charts-cgpt.r` itself) resolves relative to the current working
directory, not this script's file location, and resolves to the wrong place if run from inside
`scripts/` or `scripts/analysis/` (confirmed directly: it produces a doubled
`NonCollaborative/NonCollaborative/...` path and fails to find the file).
...
- The conflict-group split (target spans [5–13] and [6–17]) is hardcoded as the default for
  nyan1308. The generator for `laminar_conflict_groups.r`, `laminar_four_trees.r`,
  `laminar_freqtree.r`, and `laminar_spanchart.r` was never committed (the scripts were added
  alone in commit `3a39a08`); the `.r` files themselves are the only source. Their
  family-selection rules were recovered from those files — see
...
CCDB test data, `NonCollaborative/domains/domains_nyan1308.tsv`, by way of the bundle the
exporter writes. PDFs are written to `NonCollaborative/results/`.

To render:
```
Rscript NonCollaborative/scripts/render_planarsviz.R \
  --bundle NonCollaborative/results/planarsviz/nyan1308 \
  --output NonCollaborative/results --plots pooled_plot,pooled_domainplot
```
...
`NonCollaborative/OlderFiles/planarsviz_superseded/scripts/`. It is kept because the porting
...
Rscript NonCollaborative/scripts/render_planarsviz.R \
  --bundle NonCollaborative/results/planarsviz/nyan1308 \
  --output NonCollaborative/results --plots boundary_skyline
```

The hand-written script this came from, `nyan_boundary_skyline.r`, is archived at
`NonCollaborative/OlderFiles/planarsviz_superseded/scripts/` and is still run by the porting
...
### nyan1308_tree_count_all.pdf / nyan1308_tree_count_by_class.pdf / nyan1308_tree_count_without_adjacent.pdf / nyan1308_tree_counts.tsv
...
nested or disjoint) nyan1308's data supports under a few conditions, generated by
`scripts/analysis/laminar_tree_counts.py`. Reuses `laminar_analysis.py`'s own span-loading and
family-enumeration logic, so counts here are consistent with `nyan1308_laminar_analysis.md` and
the conflict/forest charts above.
...
- **`nyan1308_tree_count_by_class.pdf`** — one bar per domain class (morphosyntactic,
  ...
  constituency charts' `CLASS_COLORS` above.
- **`nyan1308_tree_count_without_adjacent.pdf`** — "all tests" vs. "without size-2 spans"
  ...
  pairs.
- **`nyan1308_tree_counts.tsv`** — the raw counts behind all three charts: `condition` (`all_tests`
  / `without_adjacent_spans`), `class` (`all` or one domain class), `n_unique_spans`,
