...
The edge-weight encoding (thickness = sqrt of how many of the 69 families share that span) makes
the consensus backbone (see "laminar_conflict_groups.r" above) visually obvious here in a way
Panel ALL's un-labeled version doesn't quite convey: the 5 always-present spans render as the
thickest, near-black lines, with the conflict-driven variation fanning out as thinner grey edges
beneath them — without needing a separate red highlight layer.

Tip-label `offset`/`vjust`/margin match the verified fix described under
`nyan1308_random_tree_overlay.r` further below — `offset=-1, vjust=1.25`, plus a widened bottom
`plot.margin` — not the plain `offset=-1` still used (as of this writing) in
`nyan1308_laminar_overlay.r`, whose gap was measured too small (39px on a comparable canvas) to
read cleanly at normal viewing scale. Fixing `generate_r_overlay_script()` itself means the same
fix now applies to any future regeneration of `nyan1308_laminar_overlay.r` too, though that file's
already-delivered `.pdf` hasn't been re-rendered as part of this change.
...
**Darkness/thickness legend** (`_legend.pdf`): both the darkness (accumulated opacity across the
69 semi-transparent layers) and thickness (`sqrt` of family count) encode the same quantity — how
many of the 69 families share a span — so one legend explains both. Each swatch renders at the
*exact* composited alpha a real stack of that many layers would reach
(`1 - (1-alphaval)**n`, matching the real chart's own accumulation formula rather than
approximating it) and the matching `sqrt(n)` linewidth, at four representative counts spanning the
full range (all `n_total`, ~50%, ~15%, and a single contingent family) computed from `n_total`
generically rather than hardcoded to nyan1308's 69. A domain-type color legend (the kind
`nyan1308_laminar_overlay.r` gets, five colors) would be meaningless on this all-black chart, so
`generate_r_overlay_script()` now branches: more than one colored group in the call → color legend
as before; exactly one → this darkness/thickness legend instead.

**Output size:** 20 × 14 in.
