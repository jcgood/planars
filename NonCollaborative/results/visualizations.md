...
`illustrations_random_tree_overlay.pdf` further below — not the plain `offset=-1` the coloured overlay
...
### illustrations_random_tree_overlay.pdf
...
no target span to highlight in a purely combinatorial sample, so every tree's edges are the same
fixed low alpha (`0.02` by default), forced black, so recurring edges accumulate visible opacity.
Tip labels ("position number" over "name") are added once, on the last tree drawn, so the label
layer sits above all 200 ghost trees rather than underneath most of them.

**Absorbed in phase E (2026-09-22), and this is the one thing about it that actually changed**:
the sample now has a recorded seed (`export_planarsviz_illustrations.py --seed`, default `0`,
written to `metadata.json`). The generated-R-script version this replaced,
`scripts/analysis/random_tree_overlay.py` (archived in
`OlderFiles/planarsviz_superseded/scripts/analysis/`), sampled 200 trees and pasted their Newick
strings straight into a 2200-line `.r` file with no seed anywhere — so that exact sample could
never be reproduced or checked, only ever looked at again. `--seed 0`'s sample is a *different*
200 trees than that file drew (there was nothing to reproduce), but is otherwise drawn by the
exact same, still-live sampler (`sample_tree()`/`sample_forest()`/`sample_labeled_tree()`, moved
to `scripts/exploratory/catalan.py` where the counting recurrence they turn into weighted choices
already lived) — verified exactly uniform by sampling 11,000 trees at n=4 (11 distinct shapes) and
confirming each appeared at ~9.09% ± noise, matching 1/11. The offset/vjust tip-label tuning that
took real measurement to get right (checked against a `geom_tippoint()` reference rather than
inferred from the coordinate system) carried over unchanged into `plot_random_tree_overlay()` —
see that function's source if the exact pixel investigation is wanted; not repeated here now that
it lives with the code it's about.
...
### illustrations_tree_count_growth.pdf

**What it is:** Catalan numbers (binary trees) and little Schröder numbers (n-ary trees, OEIS
A001003) plotted against leaf count on a log scale, with nyan1308's own 69 observed families drawn
as a dotted reference line — the chart version of the question `tree_counting_equations.tex`'s
static table raises but doesn't answer visually: how large is the tree space next to what was
actually found? A new chart, not a port — the old scripts only ever showed this comparison as a
table. Drawn by `plot_tree_count_growth()` from `tree_count_growth.tsv`
(`catalan_number()`/`all_trees_new()` in `catalan.py`, for n=1..15 by default); the reference line
is optional and comes from whichever language bundle is passed as its second argument, not a
literal 69 in the chart code.
...
- `illustrations_tree_shapes_n2.pdf`, `_n3.pdf`, `_n4.pdf`, `_n5.pdf` —
  ...
  in `../REFERENCES.md`, not analyses of any language. Drawn by
  `plot_tree_shapes(ref, n)` from the `illustrations` bundle's
  `tree_shapes.tsv`. Absorbed in phase E (2026-09-22) from the standalone
  `scripts/exploratory/generate_supercatalan_rows.py` +
  `render_supercatalan_rows.r` pair (archived in
  `OlderFiles/planarsviz_superseded/scripts/exploratory/`, along with the
  intermediate `supercatalan_trees.json` they used to pass data between
  Python and R) — checked against those two at 0.0000% differing pixels
  before they were archived. One deliberate difference: the old pair ran
  `pdfcrop` on each row afterward to trim it to content; this package never
