...
- `domains_<Planar_ID>.tsv` — the 21 CCDB structures (16 verbal, 5 nominal; e.g. `domains_chac1251_verbal.tsv`), written by `scripts/analysis/import_ccdb.py`
...
- `planar_stan1293.tsv` — Canonical planar structure for Standard English (49 positions)
- `planar_nyan1308.tsv` — Planar structure for Chichewa
- `planar_<Planar_ID>.tsv` / `ccdb_<Planar_ID>.json` — the same 21 CCDB structures' planar tables and import facts (root position, CCDB commit), written by `scripts/analysis/import_ccdb.py`
- `chart_settings_<dataset>.json` — optional hand-set chart settings for a language not from CCDB (display name, groupings, root position, planar type, forest axis), read by `scripts/planarsviz_language.py`
...
- **`scripts/analysis/`** — the active analysis pipeline, now run over Chichewa/nyan1308 and the 21 CCDB structures alike: `laminar_analysis.py` (core laminar-family enumeration engine), `laminar_tree_counts.py` (family counts, pooled and per class), `boundary_strength.py` (per-juncture boundary strength), `class_fragmentation_test.py` (is a class more fragmented than its test count predicts? — its charts come from the package), `span_placement_test.py` and `arbitrary_layers_test.py` (the project's two other permutation nulls, each weaker than the last — see `arbitrary_layers_test.py`'s docstring for how the three relate), `refinement_counts.py` (how much tree structure each family leaves open), `planars_groupings.py` (the named domain-type bundles and filters, defined once), `import_ccdb.py` (reads the local CCDB clone and writes each structure's `domains/`, `planar_tables/` and `ccdb_<Planar_ID>.json` files — the one place that reads CCDB), `export_planarsviz_illustrations.py` (writes the one bundle with no language behind it: tree shapes, Catalan/little-Schröder count growth, and a uniformly-random tree sample, phase E, 2026-09-22), `export_cross_language.py` (writes the `cross_language` bundle that sets all 22 structures side by side, by reading their bundles — no analysis re-run, 2026-09-23), and `export_planarsviz_data.py` (writes the data bundle the R package draws from). See `scripts/README_laminar_analysis.md` for the full walkthrough and `scripts/INDEX.md` for a per-script index.
...
- `planarsviz_tutorial.md` — a walk-through of one language (Chácobo verbal) from nothing to finished charts, in order; the guide is the reference it links to.
...
- `PLAN_planarsviz_library.md` / `PLANARSVIZ_LIBRARY_PROGRESS.md` — why the library is built as it is, and the chart-by-chart record of how each port was checked, with open questions. **`PLANARSVIZ_LIBRARY_PROGRESS.md` is the current state of that work** — read it before resuming. `PLAN_planarsviz_refactor.md` is this plan's predecessor (superseded execution approach; goals/architecture still stand).
...
- `PLAN_cross_language_charts.md` / `CROSS_LANGUAGE_PROGRESS.md` — charts and a summary table setting all 22 structures side by side, bearing on the three hypotheses (built 2026-09-23: `scripts/analysis/export_cross_language.py`, `planarsviz/R/cross_language.R`, catalogue §19). **`CROSS_LANGUAGE_PROGRESS.md` is the current state of that work.**
...
- `NOTE_FOR_*.md`, `PLANARSVIZ_REVIEW_FEEDBACK.md` — one-off handoffs and a review report from specific past sessions, not maintained as reference; read only if pointed here.
...
- `test_planarsviz_bundle.py` and `test_planarsviz_shifted_bundle.py` check the exported data bundles; `test_planarsviz_cross_language_bundle.py` checks the cross-language bundle against the per-language ones (and fails when one of those has been re-exported since).
