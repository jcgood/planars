...
### `coding/generate_biuniqueness_allomorphy_sheet.py` (291 lines)
...
| Operation | Call sites | Args / return usage | Retry | R/W |
|---|---|---|---|---|
| `drive.files().list(...)` / `gc.create(...)` / `_move_to_folder(...)` | `:149-161` (`_get_or_create_allomorphy_spreadsheet`) | Identical shape to `generate_status_sheet.py:284-296`'s `_get_or_create_status_spreadsheet` — **fourth** independent "find-by-name-in-folder" implementation in the inventory | No | Read then maybe Write |
...
| `_load_manifest_from_drive` / `_upload_planars_config` | `:265, :284` | Same end-of-run manifest-sync pattern (`biuniqueness_allomorphy_spreadsheet_id`/`url` fields), gated on the value actually changing | No (inherited) | Read / Write |
...
- `update(worksheet, values: List[List[str]], range_start: str = "A1", raw: bool = True)`
  **[W]** — covers every `ws.update(...)` call site. **`range_start` is an
  ...
  `generate_biuniqueness_allomorphy_sheet.py` pass `raw=False` so embedded
...
`generate_status_sheet.py` and `generate_biuniqueness_allomorphy_sheet.py`) is a
...
`generate_biuniqueness_allomorphy_sheet.py`) and `updateCells` (per-cell,
