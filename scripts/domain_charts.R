...
group.colors <- c(length = "#BC3C29", morphosyntactic = "#0072B5", phonological ="#E18727", tonosegmental = "#20845E", intonational = "#7876B1")


# set up custom plot function, messed around with reorder to do some custom sorting. Will need to keep working with
constituency.plot <- function(c, b, o){
  ggplot(c, aes(x = Edge,
  #y = reorder(Test_Labels, desc(Size*100 + as.numeric(Layer))), label = Layer)) +
...
tests_intonational <- filter(domains, Domain_Type == "intonational")
tests_plot_intonational <- df.plot(tests_intonational)
