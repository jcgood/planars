...
docstring for why _get_or_create_stage1_spreadsheet/_write_stage1_sheet are
...
def test_build_stage1_rows_excludes_excluded_scope():
    df = _planar_df([
        ("v:verbstem", "KEYSTONE", "excluded"),
        ("v:npsubj1", "NP{S,A}, he", "excluded, filled"),
    ])
    rows = _build_stage1_rows(df)
    assert rows == [{"position_name": "v:npsubj1", "element": "he", "scope": "filled"}]


def test_build_stage1_rows_keeps_filled_and_open_category():
    df = _planar_df([
...
def test_build_stage1_rows_missing_column_raises():
    df = pd.DataFrame([{"Position_Name": "v:leftedge", "Elements": "and"}])
    with pytest.raises(ValueError, match="Biuniqueness_Scope"):
        _build_stage1_rows(df)


def test_build_stage1_rows_token_count_mismatch_raises():
    df = _planar_df([("v:leftedge", "and, then", "filled")])  # only one scope, two elements
    with pytest.raises(ValueError, match="stage1|drifted|token"):
        _build_stage1_rows(df)


def test_build_stage1_rows_empty_elements_yields_no_rows():
    df = _planar_df([("v:leftedge", "", "")])
    assert _build_stage1_rows(df) == []
