...
2. **Manual annotation**: Specialists fill in y/n values, either in blank TSVs directly or via Google Sheets forms generated by `generate_sheets.py`. The Sheets workflow creates one file per analysis class with one tab per construction, y/n dropdown validation, and a shared Drive folder. OAuth credentials must be at `~/.config/planars/oauth_credentials.json` (set `PLANARS_OAUTH_CREDENTIALS` to override). The token is cached after first authorization. `sheets_manifest.json` (gitignored) records the folder and sheet URLs for the current run.
