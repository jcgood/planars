...
# Sync param columns when diagnostics_{lang_id}.tsv changes (dry run by default)
...
- `sync_params.py`: `sync-params` — syncs param columns when `diagnostics_{lang_id}.tsv` changes.
...
- Which parameters must appear in diagnostics_{lang_id}.tsv for each class
