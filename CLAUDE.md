...
- `ciscategorial.py`, `subspanrepetition.py`, `noninterruption.py`, `stress.py`, `aspiration.py`, `nonpermutability.py`, `coreference.py`, `free_occurrence.py`, `biuniqueness.py`, `repair.py`, `segmental.py`, `metrical.py`, `tonal.py`, `tonosegmental.py`, `intonational.py`, `pausing.py`, `proform.py`, `play_language.py`, `idiom.py`: each exposes `derive_*()` (takes an optional `Path` or `_data` kwarg, returns a result dict) and `format_result()` (takes a result dict, returns a formatted string). `coreference.py` differs from the standard pattern: it takes pair-row TSVs (one per construction: `reflexivization`, `pronominalization`, `np_reference`) rather than element-criterion TSVs; it derives binder/bindee domains rather than complete/partial position sets; and it has a prescreening construction (`prescreening`, criterion `referential`) that feeds element sets to the three pair constructions. `proform.py` differs from the standard complete/partial pattern: criterion `shareable_proform_replace` has three values (`y`, `both`, `n`), with `y` mapping to strict qualification and `both` to loose qualification — two separate element masks feed `position_sets_from_element_mask` independently to produce complete and partial position sets.
- `reports.py`: data layer for all downstream analysis. Owns `_CLASS_HANDLERS` (analysis registry), `project_spans`/`language_spans` (span collection), `language_report_data` (full bundle for rendering), and span label constants. Also owns `_REPORTABLE_CONSTRUCTIONS` — a per-class filter that blocks non-span-producing constructions (e.g. `element_prescreening`) from being passed to derive functions; any code iterating constructions for display or analysis should apply this filter. `collect_all_spans`/`collect_all_spans_from_sheets` are backward-compat aliases.
...
- `generate_sheets.py`: Creates annotation sheets; validates planar/diagnostics first; **`--force` aborts if any language already has sheet IDs in the manifest** (annotation data is irreplaceable); each spreadsheet gets a `Status` tab with `in-progress`/`ready-for-review` dropdowns. Supports two-stage pair-sheet workflows: for **nonpermutability**, `element_prescreening` is generated first; after import and annotation, `--regen-construction nonpermutability:general` regenerates the filtered pair sheet. For **coreference**, `prescreening` (criterion `referential`) is generated first; after import, `--regen-construction coreference:reflexivization` (and `pronominalization`, `np_reference`) regenerates each pair sheet filtered to referential elements. `_regen_dependents_simple` is called automatically during `data-refresh` and skips regeneration if the dependent TSV already exists. Each pair construction uses its own criterion column: `reflexive_allowed`, `pronoun_allowed`, `np_allowed`. When position numbers have shifted (a position was inserted or deleted), pass `--pos-remap OLD_NUM:NEW_NUM` (repeatable) to carry over existing annotations; direction is recomputed from the freshly generated pairs.
