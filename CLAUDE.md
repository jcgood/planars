...
- `html_report.py`: `render_language_report(data)` — takes a `language_report_data()` bundle and returns a self-contained HTML string with annotation completeness table and embedded Plotly domain chart. For non-technical collaborators (#89).
...
- `generate_reports.py`: `generate-reports` — generates and uploads `report_{lang_id}.html` directly (no Colab needed; used by the nightly GitHub Action). Uses `source="local"` from checked-out TSVs. Stores `report_html_file_id` in `drive_config.json`.
