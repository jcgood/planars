...
- `generate_sheets.py`: Creates annotation sheets; validates planar/diagnostics first; **`--force` aborts if any language already has sheet IDs in the manifest** (annotation data is irreplaceable); each spreadsheet gets a `Status` tab with `in-progress`/`ready-for-review` dropdowns. Supports a two-stage nonpermutability workflow: `element_prescreening` is generated first; after that TSV is imported and annotated, `--regen-construction nonpermutability:general` regenerates the filtered pair sheet. `_regen_dependents_simple` is called automatically during `data-refresh` and skips regeneration if the dependent TSV already exists.
