...
2. **Manual annotation**: Specialists fill in values via Google Sheets forms generated by `generate_sheets.py`. Google Sheets is the definitive copy of annotation forms. The Sheets workflow creates one file per analysis class with one tab per construction, y/n dropdown validation, and a shared Drive folder. OAuth credentials must be at `~/.config/planars/oauth_credentials.json` (set `PLANARS_OAUTH_CREDENTIALS` to override). The token is cached after first authorization. `sheets_manifest.json` (gitignored) records the folder and sheet URLs for the current run. `import_sheets.py` reads the manifest, downloads each tab, validates values (warning on blanks or unexpected entries), and writes `{construction}_filled.tsv` to `coded_data/{lang_id}/{class_name}/`. Existing files are skipped unless `--force` is passed. If any warnings are generated, they are written to `import_errors/{lang}_{timestamp}.txt` (gitignored) in addition to stdout.
